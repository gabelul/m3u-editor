    ...
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 gap-6 xl:grid-cols-3">
            <x-filament::card class="p-6 xl:col-span-1">
                <div class="flex items-start justify-between gap-4">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                            Use the dashboard to move between the installed extensions list and the plugin install queue.
                        </p>
                    </div>
                ...
                <div class="mt-4 flex flex-wrap gap-3">
                    ...
                    @if ($pluginInstallsUrl)
                        <x-filament::button tag="a" href="{{ $pluginInstallsUrl }}" color="gray" icon="heroicon-o-archive-box">
                ...
                @if ($canManagePlugins)
                    <div class="mt-4 rounded-xl border border-dashed border-gray-300 p-4 text-sm text-gray-600 dark:border-gray-700 dark:text-gray-300">
                        Header actions on this page handle discovery, browser uploads, local paths, local archives, and GitHub release staging.
                    </div>
                @else
                    <div class="mt-4 rounded-xl border border-dashed border-gray-300 p-4 text-sm text-gray-600 dark:border-gray-700 dark:text-gray-300">
            ...
            <x-filament::card class="p-6 xl:col-span-2">
                <div class="flex items-start justify-between gap-4">
                    <div>
                        <h2 class="text-lg font-semibold text-gray-900 dark:text-white">Extensions Needing Attention</h2>
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                            These extensions need review because of trust, integrity, validation, availability, or install state.
                        </p>
                    </div>
                                ...
                                <div class="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                                    <div class="min-w-0">
                                        <div class="flex flex-wrap items-center gap-2">
                                            <p class="font-medium text-gray-900 dark:text-white">{{ $plugin->name ?: $plugin->plugin_id }}</p>
                                        ...
                                        <p class="mt-2 text-sm text-gray-600 dark:text-gray-300">
                                            Trust: {{ str($plugin->trust_state ?: 'pending_review')->replace('_', ' ')->headline() }}
                                            · Integrity: {{ str($plugin->integrity_status ?: 'unknown')->replace('_', ' ')->headline() }}
                                            · Install: {{ str($plugin->installation_status ?: 'installed')->replace('_', ' ')->headline() }}
                                        </p>
                                    </div>

                                    <x-filament::button tag="a" href="{{ \App\Filament\Resources\ExtensionPlugins\ExtensionPluginResource::getUrl('edit', ['record' => $plugin]) }}" color="gray" size="sm">
                                        Open
                                    </x-filament::button>
                                </div>
                            </div>
        ...
        @if ($canManagePlugins)
            <x-filament::card class="p-6">
                <div class="flex items-start justify-between gap-4">
                    ...
                    <x-filament::button tag="a" href="{{ $pluginInstallsUrl }}" color="gray" size="sm" icon="heroicon-o-arrow-right">
                        View Queue
                    </x-filament::button>
                </div>
                        ...
                        @foreach ($recentInstallReviews as $review)
                            <div class="rounded-xl border border-gray-200 p-4 dark:border-gray-800">
                                <div class="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                                    <div class="min-w-0">
                                        <div class="flex flex-wrap items-center gap-2">
                                            <p class="font-medium text-gray-900 dark:text-white">{{ $review->plugin_name ?: $review->plugin_id ?: 'Unknown Plugin' }}</p>
                                    ...
                                    <x-filament::button tag="a" href="{{ \App\Filament\Resources\PluginInstallReviews\PluginInstallReviewResource::getUrl('edit', ['record' => $review]) }}" color="gray" size="sm">
                                        Open
                                    </x-filament::button>
                                </div>
                            </div>
